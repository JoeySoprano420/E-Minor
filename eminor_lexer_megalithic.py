#!/usr/bin/env python3
# E Minor v1.0 — Megalithic Optimized Lexer
# Compatible with eminor_grammar_v1_0.ebnf
# Modes: --json (default), --ndjson, --validate-only
...
class Token:
    __slots__ = ("kind","lexeme","line","col","value")
    def __init__(self, kind, lexeme, line, col, value=None):
        self.kind=kind; self.lexeme=lexeme; self.line=line; self.col=col; self.value=value
    def as_dict(self):
        d={"kind":self.kind,"lexeme":self.lexeme,"line":self.line,"column":self.col}
        if self.value is not None: d["value"]=self.value
        return d
    def as_nd(self):
        if self.value is None:
            return f"{self.kind}\t{self.lexeme}\t{self.line}\t{self.col}"
        return f"{self.kind}\t{self.lexeme}\t{self.line}\t{self.col}\t{self.value}"

class Lexer:
    __slots__ = ("s","n","i","line","col")
    def __init__(self, s:str):
        self.s=s; self.n=len(s); self.i=0; self.line=1; self.col=1
    ...
    def _read_ident_or_kw(self):
        ...
        if ident in KW_BOOL_LIT:
            return Token("BOOL", ident, line, col, KW_BOOL_LIT[ident])
    ...
    def _read_dollar_ident(self):
        ...
        self._adv()
        ch=self._peek()
        if not (ch and ch in ID_SET_HEAD): raise LexerError(f"Invalid identifier after $ at {line}:{col}")
    ...
    def _read_number_or_duration(self):
        ...
        lex=s[i+j-i:j]; self.i=j; self.col += (j-i)
        if unit:
        ...
        return Token("INT", s[i:j], line, col, int(num))
...
def main():
    import argparse
    ap=argparse.ArgumentParser(description="E Minor v1.0 Megalithic Lexer")
    ...
    try:
        ...
        if args.validate_only:
            lx.tokenize(sink=lambda t: None)
            print(json.dumps({"ok":True})); return
        if args.emit=="json":
            toks=lx.tokenize()
            print(json.dumps([t.as_dict() for t in toks], indent=2))
        else:
            w=sys.stdout.write
            lx.tokenize(sink=lambda t: w(t.as_nd()+"\n"))
