...
class VirtualMachine:
    ...
    def _apply_binary_op(self, op_code: int, left: Any, right: Any) -> Any:
        """Apply a binary operator to values."""
        # Find operator by code
        op = next((op for op, code in BINARY_OPS.items() if code == op_code), None)
        ...
        elif op == "/": return left / right if right != 0 else float('inf')
        elif op == "%": return left % right if right != 0 else 0
        
        raise ValueError(f"Unknown binary operator code: {op_code}")
    
    def _apply_unary_op(self, op_code: int, operand: Any) -> Any:
        """Apply a unary operator to a value."""
        # Find operator by code
        op = next((op for op, code in UNARY_OPS.items() if code == op_code), None)
        
        if op == "!": return not operand
        elif op == "~": return ~operand
        elif op == "u-": return -operand
        
        raise ValueError(f"Unknown unary operator code: {op_code}")

class BytecodeOptimizer:
    """Optimizes E Minor bytecode."""
    
    @staticmethod
    def optimize(bytecode: bytearray, symbols: Dict) -> Tuple[bytearray, Dict]:
        """Apply all optimizations to the bytecode."""
        # Apply peephole optimizations
        bytecode = BytecodeOptimizer._peephole_optimize(bytecode)
        
        # Apply constant folding
        bytecode, symbols = BytecodeOptimizer._fold_constants(bytecode, symbols)
        
        # Apply dead code elimination
        bytecode = BytecodeOptimizer._eliminate_dead_code(bytecode)
    ...
    def _peephole_optimize(bytecode: bytearray) -> bytearray:
        ...
        while i < len(bytecode):
            ...
            # Combine consecutive INIT+LOAD for same capsule
            if (i + 5 < len(bytecode) and
                bytecode[i] == OPCODES["INIT"] and
                bytecode[i+2] == OPCODES["LOAD"] and
                bytecode[i+1] == bytecode[i+3]):
                
                # Replace with a single LOAD
                result.append(bytecode[i+2])  # LOAD
                result.append(bytecode[i+3])  # capsule_id
                result.append(bytecode[i+4])  # value high byte
                result.append(bytecode[i+5])  # value low byte
                i += 6
                continue
            
            # Remove redundant PUSHK+POP sequences
            if (i + 3 < len(bytecode) and
                bytecode[i] == OPCODES["PUSHK"] and
                bytecode[i+3] == 0x70):  # Assuming 0x70 is POP
                
                # Skip this sequence
                i += 4
                continue
            
            # Copy the instruction if no optimization applies
    ...
    @staticmethod
    def _fold_constants(bytecode: bytearray, symbols: Dict) -> Tuple[bytearray, Dict]:
        ...
        while i < len(bytecode):
            # Look for PUSHK + PUSHK + BINOP sequences
            if (i + 6 < len(bytecode) and
                bytecode[i] == OPCODES["PUSHK"] and
                bytecode[i+3] == OPCODES["PUSHK"] and
                bytecode[i+6] == OPCODES["BINOP"]):
                
                # Extract constants and operation
                const1_idx = (bytecode[i+1] << 8) | bytecode[i+2]
                const2_idx = (bytecode[i+4] << 8) | bytecode[i+5]
                binop = bytecode[i+7]
                
                if const1_idx < len(constants) and const2_idx < len(constants):
                    const1 = constants[const1_idx]
                    const2 = constants[const2_idx]
                    
                    # If both constants are numbers, we can fold them
                    if const1["kind"] in ("INT", "FLOAT") and const2["kind"] in ("INT", "FLOAT"):
                        op = next((op for op, code in BINARY_OPS.items() if code == binop), None)
                        
                        # Calculate the folded result
                        result_val = None
                        if op == "+": result_val = const1["value"] + const2["value"]
                        elif op == "-": result_val = const1["value"] - const2["value"]
                        elif op == "*": result_val = const1["value"] * const2["value"]
                        elif op == "/" and const2["value"] != 0: result_val = const1["value"] / const2["value"]
                        
                        if result_val is not None:
                            # Add the result as a new constant
                            new_const_idx = len(constants)
                            constants.append({"kind": "INT" if isinstance(result_val, int) else "FLOAT", "value": result_val})
                            
                            # Replace with a single PUSHK
                            result.append(OPCODES["PUSHK"])
                            result.append((new_const_idx >> 8) & 0xFF)
                            result.append(new_const_idx & 0xFF)
                            i += 8
                            continue
    ...
    def _eliminate_dead_code(bytecode: bytearray) -> bytearray:
        ...
        while i < len(bytecode):
            # If we find an unconditional jump, mark all code until the target as dead
            if bytecode[i] == OPCODES["JMP"] and i + 2 < len(bytecode):
                offset = (bytecode[i+1] << 8) | bytecode[i+2]
                
                # Copy the jump instruction
                result.append(bytecode[i])
                result.append(bytecode[i+1])
                result.append(bytecode[i+2])
                
                # If this is a forward jump (not a loop), skip unreachable code
                if offset > 0:
                    target = i + 3 + offset
                    i = min(target, len(bytecode))
                    continue
